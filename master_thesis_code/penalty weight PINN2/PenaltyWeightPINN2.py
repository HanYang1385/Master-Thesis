...
###### W(x) = (r^\top Kr)(1+\frac{(x^\top\textbf{1}-1)^2}{\epsilon})
...
###### Penalty不能太大，即\epsilon不能取太小，可能是从损失函数的角度来看，不同空间位置的值数量级相差较大时，用平均（SGD）会增加训练难度
###### 右端项对非线性方程的影响是怎样的呢？
def compute_W_Penalty(K,Penalty_coef,X,batch,Dim):
    tensor_K = torch.from_numpy(K).float()
    batch_tK0 = tensor_K.reshape(1,Dim,Dim)
    batch_tK = batch_tK0.repeat([batch,1,1])
    batch_X = X.reshape([batch,1,Dim])
    batch_rtK = torch.einsum("ijk,ikl->ijl",batch_X,batch_tK)
    batch_rtKr = torch.einsum("ijk,ijk->ij",batch_rtK,batch_X)
    rtKr = batch_rtKr.reshape(batch)
    Penalty = 1 + Penalty_coef * (X.sum(1)-1) ** 2
    tensor_W = rtKr * Penalty
    return tensor_W
...
def compute_sqrtW_Penalty(K,Penalty_coef,X,batch,Dim):
    ...
    rtKr = batch_rtKr.reshape(batch)
    Penalty = 1 + Penalty_coef * (X.sum(1)-1) ** 2
    tensor_sqrtW = torch.sqrt(rtKr * Penalty)
    return tensor_sqrtW


###### Here FactoredEikonalLoss can be replaced by EikonalLoss
...
class Model():
    ...
    def __init__(self, ModelPath, device='cpu'):
        ...
        self.Params['Training']['Learning Rate'] = 5e-4
        self.Params['Training']['Use Scheduler (bool)'] = False
        self.Params['Training']['Penalty'] = 5
        self.Params['Training']['K'] = np.array([[0,1,2],[1,0,3],[2,3,0]])
    ...
    def train(self):
        ...
        if self.Params['Training']['Use Scheduler (bool)']:
            self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[150000, 240000], gamma=0.1)
        ...
        for iteration in range(self.Params['Training']['Number of Iterations']):
            ...
            if iteration % 1000 == 0:
                ...
                my_sol = tfix_u
                fix_abs_error = abs(real_sol-my_sol)
                fix_relate_error = fix_abs_error / real_sol
                ...
                verify_abs_error = abs(tverify_dis-tverify_u)
                verify_relate_error = verify_abs_error / tverify_dis
...
def continue_train(load_filepath,continu_trainpath,learning_rate,BSize,Dim,Iters,K,Penalty_coef,M):
    ...
    list_continue_fix_mean_rel_error = []
    list_continue_verify_mean_rel_error = []
    for iteration in range(Iters):
        ...
        if iteration % 1000 == 0:
            ...
            verify_output = continuNN(tverify_Xp)
            tfix_u = norm_fix_Xsr * fix_output
            print('tfix_u is:\n', tfix_u)
            ...
            real_sol = tdis_totall
            my_sol = tfix_u
            fix_relate_error = abs(real_sol-my_sol) / real_sol
            print('fix_relate_error is:\n',fix_relate_error)
            list_continue_fix_mean_rel_error.append(torch.mean(fix_relate_error).item())
            print('torch.mean(fix_relate_error).item() is:',torch.mean(fix_relate_error).item())

            verify_relate_error = abs(tverify_dis-tverify_u) / tverify_dis
            print('verify_relate_error is:\n',verify_relate_error)
            list_continue_verify_mean_rel_error.append(torch.mean(verify_relate_error).item())
            print('torch.mean(verify_relate_error).item() is:',torch.mean(verify_relate_error).item())
        ...
        if iteration == Iters - 1:
            with torch.no_grad():
                save(continuNN,continuOpt,list_continue_train_loss,list_continue_fix_mean_rel_error,continu_trainpath,iteration=iteration)

    return list_continue_train_loss, list_continue_fix_mean_rel_error, list_continue_verify_mean_rel_error, tdis_totall, tfix_Xs, tfix_Xr, fix_batch, tfix_Xp, norm_fix_Xsr, tverify_Xs, tverify_Xr, verify_batch, tverify_Xp, norm_verify_Xsr, tverify_dis
...
# Penalty_coef = 10
# M = 100
# list_continue_train_loss, list_continue_fix_mean_rel_error, list_continue_verify_mean_rel_error, tdis_totall, tfix_Xs, tfix_Xr, fix_batch, tfix_Xp, norm_fix_Xsr, tverify_Xs, tverify_Xr, verify_batch, tverify_Xp, norm_verify_Xsr, tverify_dis = continue_train(load_filepath,continu_trainpath,learning_rate,BSize,Dim,Iters,K,Penalty_coef,M)



# ###### Check
# load_filepath = '...\代码\权重惩罚PINN2\文章中实验二训练好的神经网络\Model_Iteration_79999_loss_0.08777207136154175.pt'
# Dim = 3
# M = 80
# print('M is:',M)
# K = np.array([[0,1,2],[1,0,3],[2,3,0]])
...
# # [tfix_Xs,tfix_Xr] = torch.split(tfix_Xp,Dim,1)
...
# fix_rel_error = abs(np_dis_totall-np_fixcheck) / np_dis_totall
#
# # np_norm_fix_Xsr = norm_fix_Xsr.numpy()
# # np_fix_Xp = tfix_Xp.numpy()
#
# ## 验证2：端点处验证
...
# verify_rel_error = abs(np_verify_dis - np_verifycheck) / np_verify_dis
...
# print('new_rel_error is:',new_rel_error)